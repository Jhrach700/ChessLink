...
export default function test() {
  ...
  async function loadNFTs() {    
    const provider = new ethers.providers.JsonRpcProvider("https://polygon-mumbai.infura.io/v3/a6dbd0c278ab481cbd1326a6386fc84a")
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pt-4">
          {
            nfts.map((nft, i) => (
                <div key={i} className="border shadow rounded-xl overflow-hidden  bg-black" >
                  <div className="flex justify-center font-mono p-1.5 text-white">{nft.fen.split(" ")[1] === 'w' ? <div>{nft.black_player} ({nft.black_elo})</div> : <div>{nft.white_player} ({nft.white_elo})</div>} </div>
                    ...
                    <div className="flex justify-center">
                  <button onClick={() => buyNft(nft)} className="flex justify-center font-bold mt-4 bg-black text-white rounded p-4 shadow-lg border-2 border-white-800 border-opacity-100">
                        Buy
                    </button>
                </div>
            ...
            <div className="p-1 pt-2">
                ...
                <p className="text-xs mb-4 font-bold text-white text-center">Owned by {nft.orig_minter.slice(0,7) + "..." + nft.orig_minter.slice(nft.orig_minter.length - 5, nft.orig_minter.length)} </p>
